...
void loop() {
  for (uint8_t i = 0; i < SONAR_NUM; i++) { // Loop through all the sensors.
    if (millis() >= pingTimer[i]) {         // Is it this sensor's time to ping?
      ...
      cm[currentSensor] = MAX_DISTANCE+2;         // Make distance large in case there's no ping echo for this sensor.
...
void echoCheck() { // If ping received, set the sensor distance to array.
  ...
  else{//timeout: distance too long or nothing detected
     cm[currentSensor] = MAX_DISTANCE+1;
  }
}
